                            ...
                            <table class="w-full">
                                ...
                                <tbody>
                                    @foreach ($user as $users)
                                        <tr>
                                            <td class="py-2 px-4 border-b text-center whitespace-nowrap">{{ $users->name }}
                                            </td>
                                            <td class="py-2 px-4 border-b text-center whitespace-nowrap">{{ $users->email }}
                                            </td>
                                            <td class="py-2 px-4 border-b text-center whitespace-nowrap">
                                                <form method="POST"
                                                    action="{{ route('roles.asignar-rol', ['user' => $users->id]) }}">
                                                    @csrf
                                                    @method('PUT')
                                                    <select name="rol_id">
                                                        @foreach ($roles as $rol)
                                                            <option value="{{ $rol->name }}"
                                                                {{ $users->hasRole($rol->name) ? 'selected' : '' }}>
                                                                {{ $rol->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
